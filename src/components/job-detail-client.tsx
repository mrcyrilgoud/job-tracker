...
export function JobDetailClient({ jobId, initial }: { jobId: string; initial: Initial }) {
  const router = useRouter();

  // `saved` mirrors what the server holds; `draft` is what the person sees.
  ...
  // Checking the posting is the only thing that moves these, and it happens
  // here, so local state stays authoritative for the life of the card.
  ...
  async function checkPosting() {
    ...
    try {
      ...
      setPosting({
        state: data.postingState as PostingState,
        lastCheckedAt: data.lastCheckedAt ?? new Date().toISOString(),
        lastCheckResult: data.lastCheckResult ?? null,
      });
      router.refresh();
    } catch (err) {
