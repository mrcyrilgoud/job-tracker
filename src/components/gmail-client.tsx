...
export function GmailClient({
  ...
  async function configure() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error ?? "Configure failed");
  ...
  async function triage(matchId: string, jobId: string | null) {
    ...
    try {
      const response = await fetch("/api/gmail/triage", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ matchId, jobId }),
      });
      const data = await response.json();
      ...
      <div className="card flex flex-wrap items-center justify-between gap-3 p-5">
        ...
        {connected ? (
          <div className="flex gap-2">
            <button onClick={poll} disabled={busy} className="btn btn-primary">
              Check inbox now
            </button>
